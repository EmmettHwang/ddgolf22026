...
| 버전 | 날짜 | 내용 |
|------|------|------|
| 4.1.20260915.1650 | 2026-09-15 | 로그인 실패 제한(5회→15분 차단+문자), 담당자 계정 `ddgolf` 분리, 유지관리 화면을 관리자 탭 안으로, SMS 관리 비활성+요금 안내, 사용량 표시, 서비스워커 캐시 제외 |
| 4.0.20260915.1440 | 2026-09-15 | 홈페이지유지관리비 화면 — 가상계좌 감춤(미계약), 영수증 모달+인쇄, 구매자 정보, 안내 문자 번호 칸(관리자만 저장), cafe24 대비 절약표, 할인 13% |
...
⚠️ **아이디·비밀번호를 이 파일에 적지 않는다.** 저장소에 그대로 올라가고
   커밋 이력에도 남아서, 나중에 지워도 이력에서는 꺼낼 수 있다.
   (2026-09-13 에 평문으로 적혀 있던 것을 지웠다. **이력에는 그대로 남아 있으므로
   그 비밀번호는 더 이상 쓰지 않는다.** 2026-09-15 에 계정 이름을 `root` 로 바꾸고
   비밀번호도 새로 바꿨다 — 옛 비밀번호로는 로그인되지 않는 것까지 확인했다)

계정은 서버 관리자에게 물어본다. 로그인은 **이메일 또는 이름** 둘 다 된다
(`accounts/views.py` 의 `CustomTokenObtainPairSerializer`).
